...
**解決策: 状態を 1 本の `uint32` に畳み込む。** さらにこの `uint32` は、後述する楽観的ロック用の版番号（`uint32`）と合わせて Redis の 64-bit 値として 1 key に収める。まず設計から始める。
...
読み書きは `BITFIELD` コマンド 1 回で完結する。Lua スクリプトでは `KEYS[1]` がチャレンジの属性を保持するハッシュキー、`KEYS[2]` が状態と版番号だけを持つ BITFIELD 専用キーになっている。役割を分離することで、状態の読み書きを hash の全フィールド取得と切り離せる。

```lua
-- 読み出し: 状態と版番号を 1 RTT で取得（KEYS[2] = BITFIELD 専用キー）
...
Lua 実装は図のバリデーション順に従う。

```lua
-- Go 側の const と 1:1 対応するビット定数をスクリプト冒頭で宣言する。
-- Go の iota 定義が変わった場合はここも合わせて変更する（単一の変更箇所）。
local BIT_LIVE           = 1   -- MFAChallengeStateLive
local BIT_MODE_PASSKEY   = 2   -- MFAChallengeStateModePasskey
local BIT_MODE_PUSH      = 4   -- MFAChallengeStateModePush
local BIT_PUSH_APPROVED  = 8   -- MFAChallengeStatePushApproved
local BIT_PUSH_DENIED    = 16  -- MFAChallengeStatePushDenied
local BIT_PASSKEY_BOUND  = 32  -- MFAChallengeStatePasskeySessionBound

if bit.band(next_state, BIT_LIVE) == 0 then return {-3, cur_ver} end
...
if push_decision == (BIT_PUSH_APPROVED + BIT_PUSH_DENIED) then return {-3, cur_ver} end
...
マジックナンバーをスクリプト冒頭の定数として宣言することで、Go 側の `const` との対応が一目で分かる。Lua には型システムがないため、コメントで Go の定数名を明示して保守性を確保している。
...
状態をビットマスク 1 本に畳み込むと、「どんな状態が有効か」の定義が明確な状態空間（State Space）として表現できる。Go 側の正規化関数と Lua 側のバリデーションが同じビットレイアウトを共有するため、言語をまたいだ一貫性が保ちやすい。
